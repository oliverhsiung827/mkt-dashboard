...
import { collection, addDoc, onSnapshot, updateDoc, doc, deleteDoc, query, where, getDocs } from "https://www.gstatic.com/firebasejs/10.7.1/firebase-firestore.js";
    ...
    methods: {
        ...
        checkDailyTasks() {
            ...
            myTasks.forEach(item => {
                ...
                if (sp.endDate === today) {
                    this.sendBrowserNotification("今日截止提醒", `通知原因：工作「${sp.title}」今日截止，請確認進度。`, `today-${sp.id}`);
                } else if (sp.endDate < today) {
                    const days = Math.floor((new Date(today) - new Date(sp.endDate)) / 86400000);
                    this.sendBrowserNotification("逾期處理提醒", `通知原因：工作「${sp.title}」已逾期 ${days} 天尚未處理完成。`, `overdue-${sp.id}`);
                }
            });
        ...
        async editParentTitle() {
            ...
            if (newTitle && newTitle.trim() !== "" && newTitle !== this.currentParentProject.title) {
                ...
                await updateDoc(doc(db, "projects", this.currentParentProject.id), { title: newTitle });
            }
        },

        async editSubProjectTitle() {
            const newTitle = prompt("修改子專案名稱:", this.currentSubProject.title);
            if (newTitle && newTitle.trim() !== "" && newTitle !== this.currentSubProject.title) {
                this.currentSubProject.title = newTitle;
                await updateDoc(doc(db, "sub_projects", this.currentSubProject.id), { title: newTitle });
            }
        },
            ...
            try {
                ...
                uniqueNames.forEach(async name => {
                    ...
                    if (targetUser && targetUser.name !== this.currentUser.name) {
                        await this.sendNotification(targetUser.name, 'task', `${this.currentUser.name} 在留言中提及了您: ${content}`, this.currentParentProject.id, this.currentSubProject.id);
                    }
                });
        ...
        async saveEvent() {
            ...
            if (new Date(this.eventForm.date) < new Date(this.currentSubProject.startDate)) {
                alert(`工作日誌日期 (${this.eventForm.date}) 不可早於子專案開始日 (${this.currentSubProject.startDate})`);
                return;
            }
            if (this.currentSubProject.events && this.currentSubProject.events.length > 0) {
                ...
                if (new Date(this.eventForm.date) < new Date(lastEventDate)) {
                    alert(`工作日誌日期 (${this.eventForm.date}) 不得早於最後一筆記錄日期 (${lastEventDate})`);
                    return;
                }
            }
            ...
            try {
                ...
                if (isHandoff) {
                    ...
                    this.sendNotification(nextHandler, 'handoff', `收到工作交接: ${this.currentSubProject.title}`, this.currentParentProject.id, this.currentSubProject.id);
                }
        ...
        openParentAbortModal() { this.modalMode = 'parent_abort'; this.delayForm = { reason: '策略調整', remark: '' }; this.showDelayReasonModal = true; },
        openSubAbortModal() { this.modalMode = 'sub_abort'; this.delayForm = { reason: '策略調整', remark: '' }; this.showDelayReasonModal = true; },
